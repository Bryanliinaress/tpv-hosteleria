      ...
      <div onClick={e => e.stopPropagation()} style={hoja}>
        ...
        <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
          {[{ v: 0, t: 'Sin dto.' }, { v: 5, t: '5%' }, { v: 10, t: '10%' }, { v: 100, t: '🎁 Invitación' }].map(o => (
            <button key={o.v} onClick={() => setDtoPct(o.v)} style={btn(dtoPct === o.v ? 'var(--color-accent)' : 'var(--color-surface-3)', { fontSize: '0.78rem', padding: '0.35rem 0.7rem' })}>{o.t}</button>
          ))}
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: '1.25rem', margin: '0.4rem 0 0.8rem' }}>
          <span>A cobrar</span><span style={{ color: 'var(--color-accent)' }}>{aCobrar.toFixed(2)} €</span>
        </div>
        ...
        {metodo === 'mixto' && (
          <div style={{ background: 'var(--color-inset)', border: '1px solid var(--color-border)', borderRadius: '0.6rem', padding: '0.7rem', marginBottom: '0.7rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem', marginTop: '0.55rem', color: 'var(--color-muted)' }}>
              ...
              <strong style={{ color: 'var(--color-accent)' }}>{aCobrar.toFixed(2)} €</strong>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.7rem' }}>
          <button onClick={() => setN(v => Math.max(1, v - 1))} style={btn('var(--color-surface-3)', { padding: '0.3rem 0.8rem' })}>−</button>
          <span style={{ fontWeight: 700, minWidth: '1.5rem', textAlign: 'center' }}>{n}</span>
          <button onClick={() => setN(v => v + 1)} style={btn('var(--color-surface-3)', { padding: '0.3rem 0.8rem' })}>+</button>
          <span style={{ marginLeft: 'auto', fontSize: '0.9rem' }}><strong style={{ color: 'var(--color-accent)' }}>{porPersona.toFixed(2)} €</strong> / persona</span>
        </div>
        ...
        {metodo === 'efectivo' && (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.5rem' }}>
              <input value={efectivo} onChange={e => setEfectivo(e.target.value)} inputMode="decimal" placeholder="Pagan con… €" style={{ flex: 1, background: 'var(--color-inset)', border: '1px solid var(--color-border)', borderRadius: '0.5rem', padding: '0.6rem 0.8rem', color: 'var(--color-text)', fontSize: '0.95rem' }} />
